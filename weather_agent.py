...
class ConstructionWeatherAgent:
    ...
    def analyze_site_risks(self):
        try:
            ...
            if rain_chance >= 80:
                risk_score += 60
                reasons.append(f"Very High Rain Forecast ({rain_chance}%)")
                ...
                risk_score += 20
                reasons.append(f"Moderate Rain Forecast ({rain_chance}%)")
                recommendations.append("MONITOR CONCRETE POURING: Rain may affect curing conditions.")
            elif rain_chance >= 10:
                risk_score += 10
                reasons.append(f"Light Rain Forecast ({rain_chance}%)")
                recommendations.append("MONITOR CONDITIONS: Light rain expected, low impact expected.")
