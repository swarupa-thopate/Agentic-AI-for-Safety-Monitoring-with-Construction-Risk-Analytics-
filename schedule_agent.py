...
def evaluate_weather_delay(weather):
    ...
    if weather["is_weather_sensitive_task"]:
        rain = weather["rain_forecast_pct"]
        if rain >= 80:
            delay_days += 1.5
            reasons.append(f"Very high rain forecast ({rain}%)")
        elif rain >= 60:
            delay_days += 1
            reasons.append(f"High rain forecast ({rain}%)")
        elif rain >= 10:
            delay_days += 0.5
            reasons.append(f"Moderate rain forecast ({rain}%)")
